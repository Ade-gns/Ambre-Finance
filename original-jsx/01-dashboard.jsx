/* Dashboard — "Atelier · Clair"
   Même layout cockpit que la version sombre, transposé en palette ambre/crème.
   1440 × 900. */

function DashAtelierClair() {
  const { CATEGORIES, MONTHLY, TRANSACTIONS } = window.AMBRE_DATA;
  ...
  return (
    <div className="atc-root">
      <style>{`
        .atc-root { width: 1440px; height: 900px; background: #efe7d6; color: var(--ink-800);
                    display: grid; grid-template-columns: 72px 1fr; font-size: 13px; }

        /* Sidebar — icons only */
        .atc-side { background: var(--cream-50); border-right: 1px solid var(--line);
                    display: flex; flex-direction: column; padding: 20px 0; align-items: center; gap: 8px; }
        .atc-logo { width: 38px; height: 38px; border-radius: 10px;
                    background: linear-gradient(140deg, #cd8459, #b8693d);
                    color: var(--cream-50); display: flex; align-items: center; justify-content: center;
                    font-family: var(--font-display); font-size: 22px; font-style: italic; margin-bottom: 12px; }
        .atc-nav-btn { width: 40px; height: 40px; border-radius: 10px; display: flex; align-items: center; justify-content: center;
                       color: var(--ink-500); position: relative; }
        .atc-nav-btn.active { background: var(--amber-100); color: var(--amber-500); }
        .atc-nav-btn .bdg { position: absolute; top: 6px; right: 6px; width: 6px; height: 6px;
                            border-radius: 999px; background: var(--rose-500); }
        .atc-side-foot { margin-top: auto; display: flex; flex-direction: column; gap: 8px; align-items: center; }

        /* Main */
        .atc-main { padding: 22px 28px; display: flex; flex-direction: column; gap: 14px; overflow: hidden; }
        .atc-top { display: flex; align-items: center; justify-content: space-between; }
        .atc-bread { display: flex; align-items: center; gap: 8px; font-size: 11px; color: var(--ink-500);
                     letter-spacing: 0.06em; text-transform: uppercase; }
        .atc-bread strong { color: var(--ink-800); font-weight: 500; letter-spacing: 0; text-transform: none; }
        .atc-tool { display: flex; gap: 8px; align-items: center; }
        .atc-btn { display: inline-flex; align-items: center; gap: 6px; padding: 7px 12px;
                   border: 1px solid var(--line); border-radius: 8px; background: var(--cream-50); color: var(--ink-700);
                   font-size: 12px; }
        .atc-btn.amber { background: var(--amber-500); color: var(--cream-50); border-color: var(--amber-500); font-weight: 500; }

        /* Hero */
        .atc-hero { display: grid; grid-template-columns: 1.1fr 0.9fr 0.9fr 0.9fr; gap: 12px; }
        .atc-card { background: var(--cream-50); border: 1px solid var(--line); border-radius: 14px; padding: 18px 20px; }
        .atc-hero-big { display: flex; flex-direction: column; gap: 8px; justify-content: space-between; min-height: 168px; }
        .atc-hero-l { font-size: 11px; color: var(--ink-500); letter-spacing: 0.1em; text-transform: uppercase; }
        .atc-hero-v { font-family: var(--font-display); font-size: 52px; line-height: 1; color: var(--ink-900); letter-spacing: -0.02em; }
        ...
        .atc-pill { display: inline-flex; align-items: center; gap: 5px; padding: 4px 10px; border-radius: 999px;
                    background: var(--cream-200); font-size: 11px; color: var(--ink-700); }
        ...
        /* Body grid */
        .atc-body { display: grid; grid-template-columns: 1fr 1.3fr; gap: 12px; flex: 1; min-height: 0; }
        .atc-left  { display: flex; flex-direction: column; gap: 12px; min-height: 0; }
        .atc-right { display: flex; flex-direction: column; gap: 12px; min-height: 0; }

        .atc-card-h { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 12px; }
        .atc-card-t { font-size: 13px; color: var(--ink-800); font-weight: 500; }
        .atc-card-s { font-size: 11px; color: var(--ink-500); margin-top: 2px; }

        /* Categories grid */
        .atc-cats { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; }
        .atc-cat-card { padding: 12px; border-radius: 10px; background: var(--cream-100);
        ...
        .atc-cat-foot { display: flex; align-items: center; justify-content: space-between; font-size: 10px; color: var(--ink-500); }

        /* Heatmap */
        .atc-heat { display: grid; grid-template-columns: repeat(7, 1fr); gap: 4px; }
        .atc-heat-cell { aspect-ratio: 1.4 / 1; border-radius: 5px; display: flex;
                         align-items: flex-end; justify-content: flex-start;
                         padding: 4px 5px; font-size: 9px; font-family: var(--font-mono); color: var(--ink-700); }
        ...
        .atc-tl-day { display: grid; grid-template-columns: 70px 1fr; gap: 12px; padding: 8px 0;
                      border-bottom: 1px solid var(--line); }
        .atc-tl-day:last-child { border-bottom: none; }
        .atc-tl-date { font-family: var(--font-mono); font-size: 11px; color: var(--ink-500); letter-spacing: 0.05em; }
        .atc-tl-date .num { font-family: var(--font-display); font-size: 18px; color: var(--ink-900); display: block; }
        .atc-tl-items { display: flex; flex-direction: column; gap: 6px; }
        .atc-tl-row { display: grid; grid-template-columns: 18px 1fr 90px; align-items: center; gap: 8px; padding: 3px 0; font-size: 12px; }
        .atc-tl-row .lbl { color: var(--ink-800); }
        .atc-tl-row .cat { font-size: 10px; color: var(--ink-500); }
        .atc-tl-amt { font-family: var(--font-mono); text-align: right; color: var(--ink-800); }
      ...
      <aside className="atc-side">
        ...
        {navItems.map((it, i) => {
          const Ico = it.icon;
          return (
            <div key={i} className={"atc-nav-btn" + (it.active ? " active" : "")} title={it.label}>
              <Ico size={18}/>
        ...
        <div className="atc-side-foot">
          <div className="atc-nav-btn"><IcSun size={18}/></div>
          <div className="atc-nav-btn"><IcLock size={16}/></div>
        </div>
      </aside>
      ...
      <main className="atc-main">
        <div className="atc-top">
          <div>
            ...
            <h1 style={{ fontFamily: "var(--font-display)", fontSize: 26, fontWeight: 400, margin: "4px 0 0",
        ...
        <div className="atc-hero">
          <div className="atc-card atc-hero-big">
            <div>
              ...
              <div className="atc-hero-v"><span className="cur">€</span>1 739<span className="cents">,89</span></div>
            </div>
        ...
        <div className="atc-body">
          <div className="atc-left">
            <div className="atc-card" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4, fontSize: 9,
                            color: "var(--ink-500)", textTransform: "uppercase", letterSpacing: "0.08em", padding: "0 2px" }}>
              ...
              <div className="atc-heat">
                {(() => {
                  const offset = 4;
                  const cells = [];
                  for (let i = 0; i < offset; i++) cells.push(<div key={"e"+i} className="atc-heat-cell" style={{ visibility: "hidden" }}/>);
                  ...
                  dailyExp.forEach((v, i) => {
                    ...
                    if (v == null) {
                      ...
                      cells.push(
                        <div key={day} className={"atc-heat-cell" + (day === today ? " today" : "")}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11,
                            color: "var(--ink-500)", borderTop: "1px solid var(--line)", paddingTop: 10, marginTop: 4 }}>
              ...
              <div className="atc-card-h">
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  {["12 m","6 m","YTD"].map((l, i) => (
                    <button key={l} className="atc-btn"
                            style={{ padding: "3px 9px", fontSize: 10,
                                     background: i === 0 ? "var(--amber-100)" : undefined,
                                     color: i === 0 ? "var(--amber-500)" : undefined,
                                     borderColor: i === 0 ? "rgba(184,105,61,0.3)" : undefined }}>{l}</button>
                  ))}
                </div>
              </div>
              <div style={{ flex: 1 }}>
              ...
              <div className="atc-cats">
                {CATEGORIES.slice(0, 6).map(c => (
                  <div key={c.id} className="atc-cat-card">
                    ...
                    <Sparkline data={Array.from({ length: 8 }, (_, i) => c.amount * (0.6 + Math.abs(Math.sin(i * 0.9 + c.id.charCodeAt(0))) * 0.7))}
                               color={c.color} width={180} height={26}/>
                    <div className="atc-cat-foot">
                      <span>{Math.round(c.share * 100)}% du mois</span>
                      <span style={{ color: c.trend > 0 ? "var(--rose-500)" : c.trend < 0 ? "var(--sage-500)" : "var(--ink-500)" }}>
...
function AreaChartLight() {
  const { MONTHLY } = window.AMBRE_DATA;
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" style={{ display: "block", width: "100%" }}>
      ...
      {[0, 0.5, 1].map((t, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={padX - 6} y={y + 4} textAnchor="end" fontSize="9" fontFamily="var(--font-mono)" fill="rgba(61,40,23,0.5)">
      ...
      {MONTHLY.map((m, i) => (
        i % 2 === 0 && <text key={i} x={xs[i]} y={height - 6} textAnchor="middle" fontSize="9"
...
window.DashAtelierClair = DashAtelierClair;

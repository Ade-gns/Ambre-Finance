...
const fmtEUR = (n, frac = 0) =>
  new Intl.NumberFormat("fr-FR", { style: "currency", currency: "EUR", minimumFractionDigits: frac, maximumFractionDigits: frac }).format(n);

function pathSmooth(points, tension = 0.35) {
...
/* ───── Sparkline ───── */
function Sparkline({ data, width = 120, height = 32, color = "#b8693d", fill = true, strokeWidth = 1.5 }) {
...
/* ───── Donut ───── */
function Donut({ data, size = 220, thickness = 28, gap = 0.012, centerLabel, centerValue, labelColor = "#5a4530", valueColor = "#3d2817" }) {
...
function AreaChart({ data, width = 520, height = 180, color = "#b8693d", yTicks = 4, padX = 8, padY = 18, labelColor = "rgba(61,40,23,0.45)" }) {
...
/* ───── Stacked area (multi-series, expense breakdown over months) ───── */
function StackedArea({ series, labels, width = 600, height = 220, padX = 8, padY = 20, labelColor = "rgba(61,40,23,0.45)" }) {
  // series: [{ key, label, color, values: [number per month] }]
...
/* ───── Horizontal bar list ───── */
function HBarList({ items, max, width = 360, barHeight = 8, textColor = "#3d2817", labelColor = "#7a6750" }) {
...
/* ───── Simple paired bars (expense vs income) ───── */
function PairedBars({ data, width = 520, height = 180, expColor = "#b8693d", incColor = "#6b7a4f", padX = 8, padY = 20, labelColor = "rgba(61,40,23,0.45)" }) {
...
function RingGauge({ value, size = 120, thickness = 10, color = "#b8693d", track = "rgba(184,105,61,0.12)" }) {
  const r = (size - thickness) / 2;
  const c = 2 * Math.PI * r;
...
Object.assign(window, { Sparkline, Donut, AreaChart, StackedArea, HBarList, PairedBars, RingGauge, fmtEUR, pathSmooth });

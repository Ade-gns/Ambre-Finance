/* Écrans — Liste des transactions
   A · Vue par défaut (table dense filtrable + tri)
   B · Vue avec panneau détail ouvert sur une transaction
   1440 × 900 chacun. */

// ── Données enrichies pour la liste ───────────────────────────────
const TX_LIST = [
  // Semaine du 12 mai
...
const TX_RANGE = { label: "Montant ≤ −10 €", id: "amt" };
const TX_PERIOD = { label: "1 mai → aujourd'hui", id: "period" };

function txCatStyle(cat) {
  const C = window.AMBRE_DATA.CATEGORIES;
  const map = Object.fromEntries(C.map(c => [c.id, c]));
...
function TxSidebar() {
  const navItems = [
    ...
    { icon: IcList, active: true, label: "Transactions" },
  ...
  return (
    <aside className="tx-side">
      <div className="tx-logo">a</div>
      {navItems.map((it, i) => {
        const Ico = it.icon;
        return (
          <div key={i} className={"tx-nav-btn" + (it.active ? " active" : "")} title={it.label}>
            <Ico size={18}/>
      ...
      <div className="tx-side-foot">
        <div className="tx-nav-btn"><IcSun size={18}/></div>
        <div className="tx-nav-btn"><IcLock size={16}/></div>
      </div>
    </aside>
  );
}
    ...
    <>
      <div className="tx-toolbar">
        ...
        <button className="tx-btn">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h13M3 12h9M3 18h5M14 10l3 3-3 3M17 13H21"/></svg>
          Date ↓
        </button>
      </div>
...
function TxRow({ t, selected, onEdit, dense = false }) {
  const cat = txCatStyle(t.cat);
  return (
    <div className={"tx-row" + (selected ? " selected" : "") + (dense ? " dense" : "")}>
      <span className={"tx-cb" + (t.amt < -50 ? "" : "")}/>
      ...
      <button className="tx-menu">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
             strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="5" r="1"/><circle cx="12" cy="12" r="1"/><circle cx="12" cy="19" r="1"/></svg>
      </button>
    </div>
  );
}
...
const TX_STYLES = `
  .tx-root { width: 1440px; height: 900px; background: #efe7d6; color: var(--ink-800);
             display: grid; grid-template-columns: 72px 1fr; font-size: 13px; }

  /* Sidebar */
  .tx-side { background: var(--cream-50); border-right: 1px solid var(--line);
             display: flex; flex-direction: column; padding: 20px 0; align-items: center; gap: 8px; }
  .tx-logo { width: 38px; height: 38px; border-radius: 10px;
  ...
  .tx-nav-btn { width: 40px; height: 40px; border-radius: 10px;
                display: flex; align-items: center; justify-content: center;
                color: var(--ink-500); position: relative; }
  .tx-nav-btn.active { background: var(--amber-100); color: var(--amber-500); }
  .tx-nav-btn .bdg { position: absolute; top: 6px; right: 6px; width: 6px; height: 6px;
                     border-radius: 999px; background: var(--rose-500); }
  .tx-side-foot { margin-top: auto; display: flex; flex-direction: column; gap: 8px; align-items: center; }

  /* Main */
  .tx-main { padding: 22px 28px; display: flex; flex-direction: column; gap: 12px; overflow: hidden; }
  ...
  .tx-btn.amber { background: var(--amber-500); color: var(--cream-50); border-color: var(--amber-500); font-weight: 500; }
  ...
  .tx-segmented { display: flex; padding: 3px; background: var(--cream-50); border: 1px solid var(--line);
  ...
  .tx-chip.clear { color: var(--rose-500); border-color: transparent; background: transparent; cursor: pointer; }
  ...
  .tx-thead { background: var(--cream-100); border-bottom: 1px solid var(--line);
              font-size: 10px; letter-spacing: 0.08em; text-transform: uppercase; color: var(--ink-500); position: sticky; top: 0; z-index: 1; }
  ...
  .tx-group-h { padding: 10px 18px 6px; font-size: 10px; letter-spacing: 0.1em;
                text-transform: uppercase; color: var(--ink-500); background: var(--cream-50);
                border-bottom: 1px dashed var(--line); display: flex; align-items: center; gap: 10px; }
  .tx-group-h .sum { margin-left: auto; font-family: var(--font-mono); color: var(--ink-700); text-transform: none; letter-spacing: 0; }

  .tx-row { padding: 10px 18px; border-bottom: 1px dashed var(--line); position: relative; }
  ...
  .tx-date .dow { font-size: 9px; letter-spacing: 0.06em; text-transform: uppercase; color: var(--ink-500); font-family: var(--font-mono); }
  .tx-date .num { font-family: var(--font-mono); font-size: 13px; color: var(--ink-800); font-weight: 500; }
  ...
  .tx-cat-chip { display: inline-flex; align-items: center; gap: 6px; padding: 3px 9px 3px 9px;
                 border: 1px solid; border-radius: 999px; font-size: 11px; background: var(--cream-50); }

  .tx-mode { font-size: 11px; color: var(--ink-500); font-family: var(--font-mono); }
  .tx-amt { font-family: var(--font-mono); font-size: 13.5px; text-align: right; color: var(--ink-800); font-weight: 500; }
  ...
  .tx-pager > button.active { background: var(--amber-100); color: var(--amber-500); border-color: rgba(184,105,61,0.3); }

  /* DETAIL PANEL */
  .tx-detail { width: 360px; background: var(--cream-50); border-left: 1px solid var(--line);
               padding: 22px 24px 18px; display: flex; flex-direction: column; gap: 18px; overflow: auto;
               flex-shrink: 0; }
  ...
  .tx-detail-amt { font-family: var(--font-display); font-size: 44px; line-height: 1; color: var(--rose-500);
  ...
  .tx-detail-lbl { font-family: var(--font-display); font-size: 22px; color: var(--ink-900); letter-spacing: -0.01em; line-height: 1.2; }
  .tx-detail-sub { font-size: 12px; color: var(--ink-500); margin-top: 4px; font-family: var(--font-mono); }

  .tx-detail-section { display: flex; flex-direction: column; gap: 8px; padding-top: 14px; border-top: 1px solid var(--line); }
  .tx-detail-section-t { font-size: 10px; color: var(--ink-500); letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 2px; }

  .tx-field { display: flex; align-items: center; justify-content: space-between;
              padding: 8px 12px; background: var(--cream-100); border: 1px solid var(--line); border-radius: 8px; }
  ...
  .tx-similar-row { display: grid; grid-template-columns: 50px 1fr 80px; align-items: center; gap: 8px;
            ...
            {(() => {
              ...
              return groups.map(g => (
                <React.Fragment key={g.label}>
                  ...
                  {g.rows.map((t, i) => <TxRow key={g.label + i} t={t}/>)}
                </React.Fragment>
              ));
            })()}
          </div>
...
// ── Vue B — Avec panneau détail ──────────────────────────────────
function ScreenTransactionsDetail() {
  // Selected: Carrefour Market on 14/05 (index 0)
  const selectedIdx = 0;
  const tSel = TX_LIST[selectedIdx];
  const catSel = txCatStyle(tSel.cat);

  return (
    <div className="tx-root" style={{ gridTemplateColumns: "72px 1fr auto" }}>
      <style>{TX_STYLES}</style>
      <TxSidebar/>
      ...
      <main className="tx-main with-panel">
        <div className="tx-top">
          ...
          <div className="tx-h1-actions">
            <button className="tx-btn">Exporter</button>
            <button className="tx-btn amber"><IcUpload size={14}/>Importer un relevé</button>
          </div>
        </div>
        ...
        <div className="tx-table">
          ...
          <div className="tx-tbody">
            <div className="tx-group-h"><span>Cette semaine · 12 – 14 mai</span><span className="sum">−1 043,93 €</span></div>
            {TX_LIST.slice(0, 5).map((t, i) => (
              <TxRow key={i} t={t} selected={i === selectedIdx}/>
            ))}
            <div className="tx-group-h"><span>Semaine du 5 mai · 5 – 11 mai</span><span className="sum">−253,34 €</span></div>
      ...
      <aside className="tx-detail">
        <button className="tx-detail-close">
        ...
        <div className="tx-detail-section">
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            <span className="tx-chip" style={{ fontSize: 10 }}><span className="amb-dot" style={{ background: "var(--amber-500)" }}/>récurrent</span>
            <span className="tx-chip" style={{ fontSize: 10 }}>épicerie</span>
            <button className="tx-chip" style={{ fontSize: 10, color: "var(--ink-500)" }}><IcPlus size={10}/> Étiquette</button>
          </div>
        </div>

        <div className="tx-detail-section">
          <div className="tx-detail-section-t">Transactions similaires <span style={{ color: "var(--amber-500)" }}>· 5</span></div>
        ...
        <div className="tx-rule">
          <div style={{ width: 24, height: 24, borderRadius: 6, background: "var(--cream-50)",
                        display: "flex", alignItems: "center", justifyContent: "center", color: "var(--amber-500)", flexShrink: 0 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <button className="tx-btn amber tx-rule-cta" style={{ padding: "5px 10px", fontSize: 11 }}>Créer la règle</button>
          </div>
        </div>
        ...
        <div className="tx-detail-section" style={{ flexDirection: "row", gap: 8 }}>
          ...
          <button className="tx-btn ghost" style={{ fontSize: 11 }}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M9 11h6M9 15h4"/><path d="M5 7l7-5 7 5v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2z"/></svg>
            Dupliquer
          </button>
          <span style={{ flex: 1 }}/>
          <button className="tx-danger-btn">
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>
            Supprimer
          </button>
        </div>
      </aside>
...
window.ScreenTransactionsDetail = ScreenTransactionsDetail;

// ── Vue C — État vide (aucune transaction sur la période) ─────
function ScreenTransactionsEmpty() {
  return (
    <div className="tx-root">
      <style>{TX_STYLES + `
        .tx-empty-card { background: var(--cream-50); border: 1px solid var(--line); border-radius: 12px;
                         flex: 1; min-height: 0; display: flex; flex-direction: column; }
        .tx-empty-h { padding: 14px 18px; background: var(--cream-100); border-bottom: 1px solid var(--line);
                      display: grid; grid-template-columns: 24px 70px 1.6fr 60px 150px 90px 110px 24px;
                      align-items: center; gap: 14px; font-size: 10px; letter-spacing: 0.08em;
                      text-transform: uppercase; color: var(--ink-500); }
        .tx-empty-body { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center;
        ...
        .tx-empty-s { font-size: 13.5px; color: var(--ink-600); text-align: center; max-width: 480px; line-height: 1.5; }
        .tx-empty-suggest { display: flex; gap: 8px; flex-wrap: wrap; justify-content: center; margin-top: 6px; }
      ...
      <main className="tx-main">
        <div className="tx-top">
          ...
          <div className="tx-h1-actions">
            <button className="tx-btn" disabled style={{ opacity: 0.5 }}>Exporter</button>
            <button className="tx-btn amber"><IcUpload size={14}/>Importer un relevé</button>
          </div>
        </div>
        ...
        <div className="tx-empty-card">
          <div className="tx-empty-h">
            <span/><span>Date</span><span>Libellé</span><span>Compte</span>
            <span>Catégorie</span><span>Mode</span>
          ...
          <div className="tx-empty-body">
            ...
            <div className="tx-empty-suggest">
              <span style={{ fontSize: 10, color: "var(--ink-500)", letterSpacing: "0.1em", textTransform: "uppercase",
                            alignSelf: "center", marginRight: 4 }}>Suggestions</span>
...
// ── Vue D — Sélection multiple (bulk actions) ─────────────────
function ScreenTransactionsBulk() {
    ...
    <div className="tx-root">
      <style>{TX_STYLES + `
        ...
        .tx-bulk-action { display: inline-flex; align-items: center; gap: 6px; padding: 7px 12px;
                          ...
                          border: 1px solid rgba(232,224,208,0.15); border-radius: 8px; font-size: 12.5px; }
        ...
        .tx-bulk-close { margin-left: auto; width: 26px; height: 26px; border-radius: 6px;
                         background: transparent; border: none; color: var(--cream-300); display: flex; align-items: center; justify-content: center; }
        ...
        .tx-row.bulk .tx-cb::after { content: ""; position: absolute; left: 3px; top: 0px; width: 4px; height: 8px;
        ...
        .tx-cat-picker > span { padding: 4px 9px; border-radius: 999px; border: 1px solid rgba(232,224,208,0.15);
                                font-size: 11px; color: var(--cream-50); display: flex; align-items: center; gap: 5px; cursor: pointer; }
      ...
      <main className="tx-main">
        ...
        {/* BULK ACTION BAR (replaces the toolbar) */}
        <div className="tx-bulk-bar">
          <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span className="tx-cb" style={{ background: "var(--amber-500)", border: "none", position: "relative" }}>
              <span style={{ position: "absolute", left: 3, top: 0, width: 4, height: 8,
                             borderRight: "1.5px solid white", borderBottom: "1.5px solid white", transform: "rotate(45deg)" }}/>
            </span>
          ...
          <button className="tx-bulk-action">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2l2.4 7.4H22l-6 4.6 2.4 7.4-6.4-4.6L5.6 21.4 8 14 2 9.4h7.6z"/></svg>
            Créer une règle
          </button>
          <button className="tx-bulk-action">Exporter…</button>
          <button className="tx-bulk-action danger">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>
            Supprimer
          </button>
          <button className="tx-bulk-close"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M6 6l12 12M18 6L6 18"/></svg></button>
        </div>
        ...
        <div className="tx-table">
          ...
          <div className="tx-tbody">
            <div className="tx-group-h">
              ...
              <span style={{ marginLeft: "auto", fontSize: 11, color: "var(--amber-500)", fontFamily: "var(--font-mono)" }}>
            ...
            {TX_LIST.slice(0, 5).map((t, i) => {
              ...
              return (
                <div key={"a-" + i} className={"tx-row" + (sel ? " bulk" : "")}>
                  ...
                  {(() => {
                    ...
                    return (
                      <span className="tx-cat-chip" style={{ borderColor: cat.color + "55", color: cat.color }}>
                        <span className="amb-dot" style={{ background: cat.color }}/>{cat.label}
                      </span>
                    );
                  })()}
                  <span className="tx-mode">{t.mode}</span>
            ...
            {TX_LIST.slice(5, 11).map((t, i) => {
              const sel = selectedIdxs.has(i + 5);
              return (
                <div key={"b-" + i} className={"tx-row" + (sel ? " bulk" : "")}>
                  <span className="tx-cb"/>
                  <div className="tx-date">
                    <span className="dow">{t.dow}</span>
                    <span className="num">{t.d}</span>
...
window.ScreenTransactionsEmpty = ScreenTransactionsEmpty;
window.ScreenTransactionsBulk  = ScreenTransactionsBulk;

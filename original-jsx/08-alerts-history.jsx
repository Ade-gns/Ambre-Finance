/* Écran — Historique des alertes
   1440 × 900. Journal des alertes déclenchées, groupé par jour. */

function ScreenAlertsHistory() {
  const navItems = [
    ...
    { icon: IcBell, active: true, label: "Alertes", badge: 2 },
  ...
  const days = [
    {
      ...
      items: [
        { time: "14:32", kind: "threshold", name: "Loisirs · seuil 85 %",
          ...
          source: "Le Petit Café · −14,20 €", state: "unread", cta: "Ajuster le budget" },
        { time: "10:11", kind: "anomaly", name: "Transaction inhabituelle détectée",
          ...
          source: "11 mai · Carte BNP", state: "unread", cta: "Voir la transaction" },
      ]
    },
    {
      label: "Hier · 13 mai",
      items: [
        { time: "18:04", kind: "event", name: "Salaire reçu",
          ...
          source: "Compte courant BNP", state: "read", cta: "Voir le détail" },
        { time: "09:47", kind: "threshold", name: "Alimentation · seuil 90 %",
          ...
          source: "Carrefour Market · −52,34 €", state: "read", cta: "Ajuster le budget" },
      ]
    },
    {
      ...
      items: [
        { time: "Dim. 22h", kind: "event", name: "Nouvel abonnement détecté",
          ...
          source: "Spotify Premium · 10,99 €/mois", state: "read", cta: "Créer la règle" },
        { time: "Sam. 19h", kind: "duplicate", name: "Doublon potentiel",
          ...
          source: "10 mai · 22,00 € · 22,00 €", state: "read", cta: "Comparer" },
      ]
    },
    {
      label: "07 – 09 mai",
      items: [
        { time: "09/05 · 11:20", kind: "anomaly", name: "Catégorie inhabituelle",
          ...
          source: "Pharmacie de l'Hôtel · −22,50 €", state: "archived", cta: "Définir un budget" },
      ]
    },
  ];
  ...
  return (
    <div className="ah-root">
      <style>{`
        .ah-root { width: 1440px; height: 900px; background: #efe7d6; color: var(--ink-800);
                   display: grid; grid-template-columns: 72px 1fr; font-size: 13px; }

        .ah-side { background: var(--cream-50); border-right: 1px solid var(--line);
                   display: flex; flex-direction: column; padding: 20px 0; align-items: center; gap: 8px; }
        .ah-logo { width: 38px; height: 38px; border-radius: 10px;
        ...
        .ah-nav-btn { width: 40px; height: 40px; border-radius: 10px; display: flex; align-items: center; justify-content: center;
                      color: var(--ink-500); position: relative; }
        .ah-nav-btn.active { background: var(--amber-100); color: var(--amber-500); }
        .ah-nav-btn .bdg { position: absolute; top: 6px; right: 6px; width: 6px; height: 6px;
                           border-radius: 999px; background: var(--rose-500); }
        .ah-side-foot { margin-top: auto; display: flex; flex-direction: column; gap: 8px; align-items: center; }

        .ah-main { padding: 22px 28px; display: flex; flex-direction: column; gap: 14px; overflow: hidden; }
        ...
        .ah-stat { background: var(--cream-50); border: 1px solid var(--line); border-radius: 12px;
                   padding: 14px 18px; }
        .ah-stat-l { font-size: 10px; color: var(--ink-500); letter-spacing: 0.08em; text-transform: uppercase; }
        .ah-stat-v { font-family: var(--font-display); font-size: 26px; color: var(--ink-900); line-height: 1.1; margin-top: 4px; }
        .ah-stat-s { font-size: 11px; color: var(--ink-500); margin-top: 4px; font-family: var(--font-mono); }

        .ah-toolbar { display: flex; align-items: center; gap: 8px; }
        .ah-seg { display: flex; padding: 3px; background: var(--cream-50); border: 1px solid var(--line);
        ...
        /* Timeline */
        .ah-timeline { background: var(--cream-50); border: 1px solid var(--line); border-radius: 14px;
                       flex: 1; min-height: 0; overflow: auto; padding: 4px 0; }
        .ah-day { padding: 14px 24px 4px; font-size: 10px; color: var(--ink-500);
                  letter-spacing: 0.1em; text-transform: uppercase;
                  position: sticky; top: 0; background: linear-gradient(var(--cream-50) 80%, transparent);
                  z-index: 1; display: flex; align-items: center; gap: 10px; }
        .ah-day strong { color: var(--ink-800); font-weight: 500; letter-spacing: 0; text-transform: none; font-size: 12px; }
        .ah-day .line { flex: 1; height: 1px; background: var(--line); }

        .ah-row { display: grid; grid-template-columns: 60px 44px 1fr 130px 32px 32px;
                  align-items: center; gap: 14px; padding: 14px 24px; border-bottom: 1px dashed var(--line); }
        .ah-row:last-child { border-bottom: none; }
        .ah-row.unread { background: var(--amber-100); }
        .ah-row.unread::before { content: ""; position: absolute; left: 0; width: 2px; top: 0; bottom: 0;
                                  background: var(--amber-500); }
        .ah-row { position: relative; }

        .ah-time { font-family: var(--font-mono); font-size: 11px; color: var(--ink-500); }
        .ah-ico { width: 44px; height: 44px; border-radius: 11px; display: flex; align-items: center; justify-content: center; }
        .ah-name { font-size: 13.5px; color: var(--ink-900); font-weight: 500; display: flex; align-items: center; gap: 10px; }
        .ah-name .unread-dot { width: 6px; height: 6px; border-radius: 999px; background: var(--amber-500); }
        .ah-msg { font-size: 12px; color: var(--ink-600); margin-top: 3px; line-height: 1.45; }
        .ah-src { font-size: 11px; color: var(--ink-500); font-family: var(--font-mono); margin-top: 4px;
                  display: flex; align-items: center; gap: 6px; }
        .ah-src .amb-dot { width: 7px; height: 7px; }

        .ah-cta { font-size: 11px; color: var(--amber-500); background: transparent; border: 1px solid rgba(184,105,61,0.3);
                  padding: 5px 10px; border-radius: 7px; cursor: pointer; }
        .ah-iconbtn { width: 30px; height: 30px; border: 1px solid var(--line); background: var(--cream-50);
                      color: var(--ink-500); border-radius: 7px; display: flex; align-items: center; justify-content: center; padding: 0; }
      `}</style>

      <aside className="ah-side">
        <div className="ah-logo">a</div>
        {navItems.map((it, i) => {
          const Ico = it.icon;
          return (
            <div key={i} className={"ah-nav-btn" + (it.active ? " active" : "")} title={it.label}>
              <Ico size={18}/>
        ...
        <div className="ah-side-foot">
          <div className="ah-nav-btn"><IcSun size={18}/></div>
          <div className="ah-nav-btn"><IcLock size={16}/></div>
        </div>
      </aside>
          ...
          {days.map(d => (
            <div key={d.label}>
              <div className="ah-day">
                ...
                <span className="line"/>
                <span className="mono" style={{ fontFamily: "var(--font-mono)", color: "var(--ink-500)" }}>
                  {d.items.length} alerte{d.items.length > 1 ? "s" : ""}
                </span>
              </div>
              {d.items.map((a, i) => {
                const style = kindBg[a.kind];
                return (
                  <div key={i} className={"ah-row " + a.state}>
                    ...
                    <div className="ah-ico" style={{ background: style.bg, color: style.fg }}>
                      {a.kind === "threshold" && <IcBell size={18}/>}
                      {a.kind === "anomaly" && (
                      ...
                      {a.kind === "duplicate" && (
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                             strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                          <rect x="4" y="4" width="14" height="14" rx="2"/><rect x="9" y="9" width="11" height="11" rx="2"/>
                        </svg>
                      )}
                    </div>
                    <div>
                      ...
                      <div className="ah-msg" dangerouslySetInnerHTML={{ __html: a.msg }}/>
                    ...
                    <button className="ah-iconbtn" title="Marquer comme lu">
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                           strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12l6 6 10-12"/></svg>
                    </button>
...
window.ScreenAlertsHistory = ScreenAlertsHistory;

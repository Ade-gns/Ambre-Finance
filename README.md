...
### Import
Importation de relevés bancaires aux formats PDF, CSV, OFX et QIF. Aperçu dynamique des transactions avant confirmation, avec détection automatique du séparateur et du format de date pour le CSV, et gabarits dédiés à neuf banques françaises pour le PDF (repli générique pour les autres). L'OFX est lu dans ses deux dialectes (1.x SGML et 2.x XML) ; le QIF reprend la catégorie Quicken quand elle est présente.
...
- **Importer un relevé** — glisser-déposer ou sélectionner un fichier `.pdf`, `.csv`, `.ofx` ou `.qif` exporté depuis votre banque.
